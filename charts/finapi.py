...
def mcap(ticker: str):
    ...
    val = r.get(f"https://financialmodelingprep.com/api/v3/historical-market-capitalization/{ticker}?apikey={apikey}").json()
    ...
    while len(val) > 0:
        i += 1
        ret = [*ret, *(val)]
        last_date = datetime.datetime.strptime(val[-1]['date'], FMP_DATE_FORMAT) - datetime.timedelta(days=1)
        val = r.get(f"https://financialmodelingprep.com/api/v3/historical-market-capitalization/{ticker}?to={last_date.strftime(FMP_DATE_FORMAT)}&apikey={apikey}").json()
        if i > 40:
...
def _enterprise_vals(ticker: str):
    ...
    val = r.get(f"https://financialmodelingprep.com/api/v3/enterprise-values/{ticker}/?period=quarter&apikey={apikey}").json()
...
def _income_statement(ticker: str):
    apikey = get_secret(ConfigKey.FMP)
    # todo, cache this result
    val = r.get(f"https://financialmodelingprep.com/api/v3/income-statement/{ticker}?period=quarter&apikey={apikey}").json()
...
def _key_metrics(ticker: str):
    apikey = get_secret(ConfigKey.FMP)
    val = r.get(f"https://financialmodelingprep.com/api/v3/key-metrics/{ticker}?period=quarter&apikey={apikey}").json()
...
def tangible_book_value(ticker: str):
    ...
    v['tangibleBookValuePerShare'] = v['tangibleBookValuePerShare'] / v['revenuePerShare'] / v['priceToSalesRatio'] * v['marketCap']
...
def _ratios(ticker: str):
    apikey = get_secret(ConfigKey.FMP)
    val = r.get(f"https://financialmodelingprep.com/api/v3/ratios/{ticker}?period=quarter&apikey={apikey}").json()
