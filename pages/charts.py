...
def render_main():
    ...
    def get_graph4(*_, dividend_amount, start_date, **rest):
        ...
        dividend_ratios = dividend_amount.rolling(4).sum().rolling(5).apply(lambda a: (a[4] - a[0]) / a[0])
       ...
       try:
           ...
           for i, axis in enumerate(axes):
               ...
               for col in axis[0]:
                   ...
                   fig.add_trace(go.Scatter(x=graph.index, y=graph[col],hovertemplate='%{y}', name=(col if not ('shortform' in data and col in data['shortform']) else data['shortform'][col]),yaxis=f"y{label}"))
           ...
           fig.update_layout(**layoutupdate, title=f"{ticker}: {', '.join(graph.columns) if not ('title' in data) else data['title']}", hovermode="x unified", xaxis_title="Date", hoverlabel_namelength=4, margin_l = 80 + 20 * len(axes))
