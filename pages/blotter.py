...
st.title("Master Blotter, Relative Price Returns")
...
input_tickers = st.text_area(
    "Enter Tickers (comma-separated):", value=", ".join(tickers)
)
input_dates = st.text_area(
    "Enter Purchase Dates (comma-separated, YYYY-MM-DD):",
    value=", ".join(purchase_dates),
)
...
@st.cache_data
def relative_return(ticker, purchase_date, start_date_filter):
    try:
        ...
        if (
            purchase_date not in ticker_price.index
            or purchase_date not in index_price.index
        ):
            st.error(
                f"No data available from {purchase_date} for {ticker} or {index_ticker}."
            )
            return None
        ...
        elif start_date_filter == "3 Years":
            adjusted_start_date = today - timedelta(days=3 * 365)
        elif start_date_filter == "5 Years":
            adjusted_start_date = today - timedelta(days=5 * 365)
        ...
        returns_df = pd.concat(
            [index_return, ticker_return], axis=1, keys=[index_ticker, ticker]
        ).map(utils.custom_round)

        fig = go.Figure()
        fig.add_trace(
            go.Scatter(
                x=returns_df.index,
                y=returns_df[index_ticker],
                mode="lines",
                name=index_ticker,
            )
        )
        fig.add_trace(
            go.Scatter(
                x=returns_df.index, y=returns_df[ticker], mode="lines", name=ticker
            )
        )
...
for ticker, purchase_date in zip(tickers, purchase_dates):
    st.subheader(f"{ticker} (Purchased {purchase_date})")
    fig = relative_return(ticker, purchase_date, date_filter_option)
    if fig:
